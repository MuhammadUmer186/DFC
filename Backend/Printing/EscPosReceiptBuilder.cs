...
namespace RestaurantSystem.Printing
{
    public static class EscPosReceiptBuilder
    {
        ...
        public static string Build(PrintReceiptDto dto)
        {
            ...
            // ===== ORDER TYPE BANNER (delivery/online orders only) =====
            if (!string.IsNullOrWhiteSpace(dto.OrderTypeLabel))
            {
                lines.Add("\x1B\x61\x01\x1B\x45\x01\x1D\x21\x11" + $"** {dto.OrderTypeLabel} **" +
          ...
          $"ORDER NO: {dto.OrderNo}" + "\x1D\x21\x00\x1B\x45\x00");
            ...
            if (!string.IsNullOrWhiteSpace(dto.CustomerName))
                lines.Add($"Customer: {dto.CustomerName}");
            if (!string.IsNullOrWhiteSpace(dto.CustomerPhone))
                lines.Add($"WhatsApp: {dto.CustomerPhone}");
            if (!string.IsNullOrWhiteSpace(dto.CustomerAddress))
                lines.Add($"Address: {dto.CustomerAddress}");
