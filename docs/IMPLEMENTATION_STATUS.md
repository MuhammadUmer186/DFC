...
| 5 | Transactional sync (Outbox/Inbox/Checkpoint/Conflict/DeadLetter, `/api/sync/*`, HMAC) | ✅ | `SyncOutbox/Inbox/Checkpoint/Conflict/DeadLetter/Nonce` + migration. Outbox written in the SaveChanges interceptor (same tx). `AggregateSnapshotService` serialize/apply with FK→GlobalId resolution + owned-child upsert. `SyncApplyService` (idempotent inbox, version stale/gap/conflict, dead-letter, per-event ack). `SyncHmacMiddleware` (X-Sync-* signature over `METHOD\npathAndQuery\nts\nnonce\nbodyHash`, nonce replay + skew). `SyncController` (`push`/`pull`/`ack`/`heartbeat`/`status`, HMAC). `SyncAdminController` (`status`/`conflicts`/`dead-letters`/resolve/replay, JWT SuperAdmin/MainAdmin). `SyncWorker` hosted service (Edge-default, `sp_getapplock` exclusive, push→pull→ack, exp backoff+jitter). `SyncPeerClient` signed HTTP. **Verified single-node**: order create → `OrderUpserted`+`KitchenOutUpserted` outbox rows same-tx; `/api/sync/status` 401 w/o sig, 200 with; pull→clone→push → `applied` (new Order+items, MenuItem FK resolved from GlobalId), re-push same EventId → `duplicate`. **Two-node integration test pending real edge deploy.** |
